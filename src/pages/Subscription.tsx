        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full max-w-4xl">
          {plans.map((plan, index) => (
            ...
            >
              ...
              <div className="mb-10">
                <h3 className={`text-2xl font-bold mb-6 ${plan.isPaid ? 'text-white' : 'text-[#001A41]'}`}>
                  {plan.name}
                </h3>
                <div className="flex items-baseline mb-1">
                  <span className={`text-[16px] font-bold self-start mt-2 mr-0.5 ${plan.isPaid ? 'text-white' : 'text-[#001A41]'}`}>₸</span>
                  <span className={`text-6xl font-bold tracking-tight ${plan.isPaid ? 'text-white' : 'text-[#001A41]'}`}>
                    {plan.price}
                  </span>
                  <span className={`ml-2 text-[10px] uppercase font-medium ${plan.isPaid ? 'text-gray-200' : 'text-gray-500'}`}>
                    KZT /месяц
                  </span>
                </div>
                <p className={`text-sm font-medium mt-6 leading-relaxed ${plan.isPaid ? 'text-gray-100' : 'text-gray-900'}`}>
                  {plan.tagline}
                </p>
              </div>
              ...
              <div className="space-y-5">
                {plan.features.map((feature, fIndex) => (
                  <div key={fIndex} className="flex items-start gap-4">
                    <div className={`${plan.isPaid && fIndex < 3 ? 'text-white' : 'text-gray-400'} mt-0.5 flex-shrink-0`}>
                      {feature.icon}
                    </div>
                    <span className={`text-[14px] font-medium leading-snug ${plan.isPaid && fIndex < 3 ? 'text-white' : 'text-gray-700'}`}>
                      {feature.text}
                    </span>
                  </div>
                ))}
